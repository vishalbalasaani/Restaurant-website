            ...
            >
              ...
              <div className="flex flex-col md:flex-row items-center justify-end w-full md:w-auto shrink-0 gap-2 pl-2">
                ...
                <button
                  ...
                  disabled={togglingId === driver.id}
                  className={`flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold transition-all duration-300 w-full md:w-auto ${
                    driver.is_active 
                      ? 'bg-gradient-to-r from-rose-500 to-red-600 text-white shadow-lg shadow-red-500/25 hover:shadow-red-500/40 hover:-translate-y-0.5 border-none' 
                      : 'bg-gradient-to-r from-emerald-500 to-green-600 text-white shadow-lg shadow-green-500/25 hover:shadow-green-500/40 hover:-translate-y-0.5 border-none'
                  }`}
                >
                  {togglingId === driver.id ? (
                    <Loader2 className="h-4 w-4 animate-spin" />
                  ) : (
                    <Power className="h-4 w-4 drop-shadow-sm" />
                  )}
                  <span className="drop-shadow-sm">{driver.is_active ? "Mark Offline" : "Mark Online"}</span>
                </button>
              </div>
            </motion.div>
          ))}
